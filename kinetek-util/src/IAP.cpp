...
status_code IAP::put_in_iap_mode(bool forced_mode)
{
    ...
    else
    {
        ...
        sc->send_frame(KT::FORCE_ENTER_IAP_MODE_IAP, KT::enter_iap_mode_forced_data, sizeof(KT::enter_iap_mode_forced_data));
...
status_code IAP::send_init_frames()
{
    ...
    sc->send_frame(KT::FW_VERSION_REQUEST_ID | set_7th, KT::fw_version_request_data, sizeof(KT::fw_version_request_data));
    CO_CANrxMsg_t * resp = sc->get_frame(KT::FW_VERSION_RESPONSE_ID, this, resp_call_back, SHORT_WAIT_TIME, iap_can_id_mask);
...
status_code IAP::upload_hex_file()
{
    ...
    while(true) // keep sending packets until reached end of file or fail and function returns
    {
        ...
        {   
            ...
            CO_CANrxMsg_t * resp = sc->get_frame(KT::IAP_RESPONSE_ID, this, resp_call_back, MEDIUM_WAIT_TIME, iap_can_id_mask);
            ...
            // send total checksum
            sc->send_frame(KT::IAP_REQUEST_ID | set_7th, KT::calculate_total_checksum_data, sizeof(KT::calculate_total_checksum_data));
            resp = sc->get_frame(KT::IAP_RESPONSE_ID, this, resp_call_back, MEDIUM_WAIT_TIME, iap_can_id_mask);
...
status_code IAP::send_hex_packet(bool is_retry)
{
    ...
    if(is_retry)
    {
        ...
        CO_CANrxMsg_t * resp = sc->get_frame(KT::IAP_RESPONSE_ID, this, resp_call_back, SHORT_WAIT_TIME, iap_can_id_mask);
        if(KT::get_response_type(resp->ident, resp->data, resp->DLC) != KT::ACK_32_BYTES)
        {
            resp = sc->get_frame(KT::IAP_RESPONSE_ID, this, resp_call_back, SHORT_WAIT_TIME, iap_can_id_mask);
            if(KT::get_response_type(resp->ident, resp->data, resp->DLC) != KT::ACK_32_BYTES)
            {
                LOG_PRINT(("PACKET_RESENT TIMEOUT\n"));
                return PACKET_RESENT_FAIL;
            }   
        }
            ...
            else if(sum == -1) // means surpassed last data line
            {
                ...
                while(true)
                {
                    switch(curr_frame_id)
                    {
                        ...
                        {
                            usleep(1000);
                            sc->send_frame(KT::SEND_FRAME_2_ID | set_7th, current_packet+CAN_DATA_LEN*frame_count, CAN_DATA_LEN);
                        ...
                        {
                            usleep(1000);
                            sc->send_frame(KT::SEND_FRAME_3_ID | set_7th, current_packet+CAN_DATA_LEN*frame_count, CAN_DATA_LEN);
                        ...
                        {
                            usleep(1000);
                            sc->send_frame(KT::SEND_FRAME_4_ID | set_7th, current_packet+CAN_DATA_LEN*frame_count, CAN_DATA_LEN);
                            CO_CANrxMsg_t * resp = sc->get_frame(KT::IAP_RESPONSE_ID, this, resp_call_back, MEDIUM_WAIT_TIME, iap_can_id_mask);
                            if(KT::get_response_type(resp->ident, resp->data, resp->DLC) != KT::ACK_32_BYTES)
                            {
                                // try again
