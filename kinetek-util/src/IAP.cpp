...
status_code IAP::upload_hex_file()
{
    ...
    while(true) // keep sending 32 byte packets until reached end of file or fail
    {
        ...
        // every 32 packets is a page and there is a checksum
...
status_code IAP::send_hex_packet(bool is_retry)
{
    ...
    else
    {
        ...
        while(true)
        {
            if(err > 0 && err < 8) // fill last data frame for current packet
            {
                num_bytes_uploaded += last_line_data_size;
                int last_frame_filler_amount = 8 - err;
                for(int i = err-1; i < 8; i++)
                {
                    data[i] = 0xFF;
                }
            }
            ...
            if(err == -1) // means surpassed last data line
            {
